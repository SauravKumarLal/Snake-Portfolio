  ...
  return (
    <div className="project-card deploy-card" aria-hidden="true">
      ...
      <p className="deploy-cmd">
        <span className="deploy-prompt">sauravkrlal@prod:~$</span> npm run
        deploy
      </p>
      ...
      <div className="deploy-steps">
        {DEPLOY_STEPS.map((s, i) => (
          <div
            key={i}
            className={`deploy-step${steps.includes(i) ? " deploy-step--in" : ""}`}
          >
            <span className="deploy-step-text">{s.text}</span>
            <span className="deploy-step-stat">
              {s.stat} <span className="deploy-tick">✓</span>
            </span>
          </div>
        ))}
      </div>
      ...
      {live && (
        <p className="deploy-live">
          <span className="deploy-live-dot">●</span> live · serving{" "}
...
function CollaborateCard() {
  return (
    <div className="project-card collab-card">
      ...
      <a href="#contact" className="collab-cta">
        [ Get in touch ] →
      </a>
    </div>
  );
